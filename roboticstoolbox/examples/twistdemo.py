...
twist_x = swift.Slider(
    lambda x: update_twist_param(x, "x"),
    min=-1,
    max=1,
    step=0.01,
    value=0,
...
twist_y = swift.Slider(
    lambda y: update_twist_param(y, "y"),
    min=-1,
    max=1,
    step=0.01,
    value=0,
...
twist_rollangle = swift.Slider(
    lambda r: update_twist_param(np.deg2rad(r), "R"),
    min=-180,
    max=180,
    step=1,
    value=0,
...
twist_pitchangle = swift.Slider(
    lambda p: update_twist_param(np.deg2rad(p), "P"),
    min=-180,
    max=180,
    step=1,
    value=0,
...
twist_pitch = swift.Slider(
    lambda p: update_twist_param(p, "p"),
    min=0,
    max=1,
    step=0.02,
    value=0,
